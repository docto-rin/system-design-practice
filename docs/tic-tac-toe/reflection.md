...
### 依存性注入でテスタビリティを向上
レビューコメントを受けて、`Game.play()` のシグネチャを変更した。

変更前：`play()` が内部で `new Scanner(System.in)` を生成
変更後：`play(Scanner scanner, PrintStream output)` で外部から注入

これにより、
- テストで `ByteArrayInputStream` から Scanner を作成して渡せる
- Main 側で try-with-resources を使って Scanner を適切に close できる
- ライフサイクル管理の責任が明確になった

結果として、勝利条件（行・列・対角線）、引き分け、エラーハンドリング、ゲームフローの全てを自動テストできるようになった。

### コードの可読性改善
レビューで指摘された細かい点も修正：
- マジックナンバー（3）を `BOARD_SIZE` 定数に
- 条件式の可読性改善（`!(0 <= row && row < BOARD_SIZE && ...)`）
- `getDisplayString()` をヘルパーメソッドに分割（`buildColumnHeader()` など）
- テスト名を「何を・いつ・どうなる」の形式に一部修正。（本当は全てそうすべき。）
- `Board.java` について、javadoc用のコメントを書いてみた。
  - javadoc を知らなかったので実行してみた。感動した。

### プレイヤー名の入力機能追加
Main でゲーム開始前にプレイヤー名を入力できるようにした。（リファクタ前は絶妙に対応しづらかった。）同じ Scanner を使い回すことで、リソース管理も一箇所で完結している。
